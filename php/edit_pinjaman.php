...
// Ambil data pinjaman berdasarkan ID anggota dari session
$id_anggota = $user['id_anggota']; // Gunakan ID anggota dari session
$sql = "SELECT * FROM pinjaman WHERE id_anggota = ?"; // Mengambil data dari tabel pinjaman
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $pinjamanData[] = $row; // Ambil semua data pinjaman
    }
} else {
    echo "Data pinjaman tidak ditemukan.";
    exit; // Hentikan eksekusi jika tidak ada data pinjaman
}

// Misalkan kita ambil nominal dari pinjaman yang pertama
$nominal = isset($pinjamanData[0]) ? $pinjamanData[0]['nominal'] : 0; // Ambil nominal dari data pinjaman
...
<div class="card mb-4">
    ...
    <div class="card-body">
        <form id="formAccountSettings" onsubmit="updateUser(event)">
            ...
            <div class="row">
                <div class="mb-3">
                    <label for="nominal" class="form-label">Jumlah Pinjaman</label>
                    <input
                        class="form-control"
                        type="text"
                        id="nominal"
